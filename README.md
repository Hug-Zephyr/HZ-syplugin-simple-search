...
- **帮助信息按钮**: 在信息框显示区域里, 展示插件支持的所有语法, 方便在忘记的时候, 快速查看
  ![](img/help_info.png)
- **跳转按钮**: 使用默认游览器打开本插件的readme, 里面有插件最详细的介绍
- **设置按钮**: 在信息框显示区域里显示可修改的设置, 目前只支持搜索结果树相关的内容, **且设置是一次性的, 刷新思源后会重置成默认状态**
  ![](img/tree_setting.png)
  + 树 竖线风格
    ![](img/tree_style.png)
  + 搜索结果优先: 建议开启, 因为关闭会导致上下键跳转的顺序不对
    ![](img/search_res_top.png)
  + 显示全路径
    ![](img/all_path.png)
- **信息显示区**：
  + 显示帮助信息与设置
  + 在搜索后触发, 自动将插件转换后的实际搜索语句显示出来, 方便调试与确认搜索意图是否正确解析。
...
* [小优化] 设置持久化
* [小优化] 优化转换sql, 有些没必要转换
* [小功能] 增加类型: 锚文本(块引用), 行内代码, 行内公式
* [大功能] 接管上下键和回车, 输入时显示下拉框，实时更新匹配到的搜索历史, 需要测试是否可行
* [大功能] sql方式无法替换  
    方案1: 模拟替换请求
